...
def redraw_gameWindow(win, bo, p1, p2, color, ready):
    ...
    try:
        ...
        print(e)
    win.blit(txt, (490,20))
    win.blit(txt2, (480, 700))

    txt = font.render("Ấn phím q để thoát trò chơi", 1, (255, 255, 255))
    win.blit(txt, (10, 20))
    ...
    if not ready:
        show = "Vui lòng đợi người chơi khác..."
        if color == "s":
            show = "Vui lòng đợi 2 người chơi khác..."
    ...
    if not color == "s":
        font = pygame.font.SysFont("Times New Roman", 30)
        if color == "w":
            txt3 = font.render("Bạn là cờ trắng", 1, (255, 255, 0))
            win.blit(txt3, (width / 2 - txt3.get_width() / 2, 50))
        else:
            txt3 = font.render("Bạn là cờ đen", 1, (255, 255, 0))
            win.blit(txt3, (width / 2 - txt3.get_width() / 2, 50))

        if bo.turn == color:
            txt3 = font.render("Đến lượt bạn", 1, (255, 0, 0))
            win.blit(txt3, (width / 2 - txt3.get_width() / 2, 700))
        else:
            txt3 = font.render("Đến lượt nhỏ kia", 1, (255, 0, 0))
            win.blit(txt3, (width / 2 - txt3.get_width() / 2, 700))
...
def main():
    ...
    while run:
        ...
        if bo.winner == "w":
            end_screen(win, "Cờ trắng thắng rồi!!!")
            run = False
        elif bo.winner == "b":
            end_screen(win, "Cờ đen thắng rồi !!!")
            run = False
